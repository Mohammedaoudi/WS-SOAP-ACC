package ma.ensa.tpksoap2.viewModels;
        ...
        new AsyncTask<Void, Void, Compte>() {
            ...
            protected Compte doInBackground(Void... voids) {
                try {
                    ...
                    // Si la date de création est incorrecte ou null, vous pouvez la définir ici
                    if (result != null && result.getDateCreation() == null) {
                        result.setDateCreation(new Date());  // Assurez-vous d'assigner une valeur valide
                    }
                    return result;
                } catch (Exception e) {
